...
You are assigned to a team (called **Sydney**) which is developing weather related application's MVP backend.
...
### The API:
**GET .../weather**
* You can fetch the endpoint through  `https://backend-interview-sydney.tools.gcp.viesure.io/weather` with the following data structure:
```curl
curl -X 'GET' \
  'https://backend-interview-sydney.tools.gcp.viesure.io/weather' \
...
* You can call the endpoint through  `https://backend-interview-sydney.tools.gcp.viesure.io/weather/temp`
```cURL
curl -X 'PUT' \
  'https://backend-interview-sydney.tools.gcp.viesure.io/weather/temp' \
  -H 'accept: */*' \
...
* You can call the endpoint through  `https://backend-interview-sydney.tools.gcp.viesure.io/weather/condition`
```cURL
curl -X 'PUT' \
  'https://backend-interview-sydney.tools.gcp.viesure.io/weather/condition' \
  -H 'accept: */*' \
  -H 'Content-Type: application/json' \
  -d '{
  "condition": 1
